...
const char * const BITCOIN_CONF_FILENAME = "lunar.conf";
const char * const BITCOIN_PID_FILENAME = "lunard.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "lunar" is a composite category enabling all Lunar-related debug output
            if(ptrCategory->count(string("lunar"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LunarCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LunarCore
    // Mac: ~/Library/Application Support/LunarCore
    // Unix: ~/.lunarcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LunarCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LunarCore";
#else
    // Unix
    return pathRet / ".lunarcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty lunar.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override lunar.conf
